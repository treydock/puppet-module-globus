shared_examples_for 'globus::configv4' do |facts|
  ...
  context 'when first_gridftp_callback defined' do
    let(:params) { default_params.merge(first_gridftp_callback: '|globus_mapping liblcas_lcmaps_gt4_mapping.so lcmaps_callout') }
    ...
    context 'when run_setup_commands => false' do
      let(:params) { default_params.merge(first_gridftp_callback: '|globus_mapping liblcas_lcmaps_gt4_mapping.so lcmaps_callout', run_setup_commands: false) }
